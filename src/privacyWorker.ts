...
type LoadTokenClassifier = (
  task: 'token-classification',
  model: 'openai/privacy-filter',
  options: ClassifierOptions,
) => Promise<TokenClassifier>;

const loadTokenClassifier = pipeline as unknown as LoadTokenClassifier;
...
const createProgressCallback = (id: string): ProgressCallback => (progress) => {
...
const getClassifier = async (id: string) => {
  if (!classifierPromise) {
    ...
    const webgpuOptions: ClassifierOptions = {
      device: 'webgpu',
      dtype: 'q4',
      progress_callback: createProgressCallback(id),
    };

    classifierPromise = loadTokenClassifier('token-classification', 'openai/privacy-filter', webgpuOptions);
    ...
    try {
      ...
      const wasmOptions: ClassifierOptions = {
        device: 'wasm',
        dtype: 'q4',
      };

      classifierPromise = loadTokenClassifier('token-classification', 'openai/privacy-filter', wasmOptions);
    }
  }
